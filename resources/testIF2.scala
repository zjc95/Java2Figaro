...
object patch {
  def main(args: Array[String]): Unit = {
    ...
    val Var_tempA = RichCPD(Var_posX, Stmt_11_7, 
      (OneOf(true), OneOf(true)) -> Flip(0.95),
      (*, *) -> Flip(0.05))
    val Var_tempA_1 = RichCPD(Var_tempA, Stmt_11_7, 
      (OneOf(true), OneOf(true)) -> Flip(0.95),
      (*, *) -> Flip(0.05))
    val Var_tempB = RichCPD(Var_posX, Var_tempA_1, 
      (OneOf(true), OneOf(true)) -> Flip(0.95),
      (*, *) -> Flip(0.05))
    val Var_this_even = If(Var_tempA_1, Flip(0.95), Flip(0.05))
    val Var_this_size = If(Var_tempB, Flip(0.95), Flip(0.05))
    val Var_this_ret = RichCPD(Var_tempB, Var_tempA_1, 
      (OneOf(true), OneOf(true)) -> Flip(0.95),
      (*, *) -> Flip(0.05))
